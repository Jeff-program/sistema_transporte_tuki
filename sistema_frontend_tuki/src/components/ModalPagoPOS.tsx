...
const ModalPagoPOS: React.FC<ModalPagoPOSProps> = ({ 
    ...
    const esDocumentoValido = () => {
        ...
        if (tipoComprobante === 'FACTURA') {
            return documentoCliente.length === 11; 
        } else {
            ...
            return documentoCliente.length >= 5; 
        }
    };
    ...
    const handleDocumentoChange = (val: string) => {
        ...
        if (tipoComprobante === 'FACTURA') {
            limpio = val.replace(/[^0-9]/g, ''); 
        } else {
            if (tipoDocumentoBoleta === 'DNI' || tipoDocumentoBoleta === 'CARNET_EXTRANJERIA') {
                limpio = val.replace(/[^0-9]/g, '');
            } else {
                limpio = val.replace(/[^A-Za-z0-9]/g, '').toUpperCase(); 
            }
        }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-300 relative">
                
                <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-slate-200 rounded-full text-slate-500 transition-colors z-10 md:hidden shadow-sm">
                    <X size={18}/>
                </button>

                <div className="flex flex-col md:flex-row overflow-y-auto h-full" style={{ scrollbarWidth: 'thin', scrollbarColor: '#cbd5e1 transparent' }}>
                    
                    {/* PANEL IZQUIERDO: Resumen y Método de Pago */}
                    <div className="w-full md:w-1/2 bg-slate-50 p-6 border-b md:border-b-0 md:border-r border-slate-200 flex flex-col shrink-0">
                        ...
                        <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-6">
                            <button 
                                ...
                                className={`flex flex-col items-center gap-2 p-2 sm:p-3 rounded-xl border-2 transition-all ${metodoPago === 'EFECTIVO' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-white text-slate-500 hover:border-emerald-200 hover:bg-emerald-50/50'}`}
                            >
                                <Banknote size={24} />
                                <span className="text-[10px] sm:text-xs font-bold">Efectivo</span>
                            </button>
                            <button 
                                onClick={() => setMetodoPago('YAPE')}
                                className={`flex flex-col items-center gap-2 p-2 sm:p-3 rounded-xl border-2 transition-all ${metodoPago === 'YAPE' ? 'border-purple-500 bg-purple-50 text-purple-700' : 'border-slate-200 bg-white text-slate-500 hover:border-purple-200 hover:bg-purple-50/50'}`}
                            >
                                <Smartphone size={24} />
                                <span className="text-[10px] sm:text-xs font-bold">Yape/Plin</span>
                            </button>
                            <button 
                                onClick={() => setMetodoPago('TARJETA')}
                                className={`flex flex-col items-center gap-2 p-2 sm:p-3 rounded-xl border-2 transition-all ${metodoPago === 'TARJETA' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 bg-white text-slate-500 hover:border-blue-200 hover:bg-blue-50/50'}`}
                            >
                                <CreditCard size={24} />
                                <span className="text-[10px] sm:text-xs font-bold">Tarjeta</span>
                            </button>
                        </div>

                        {/* Lógica Dinámica según Método */}
                        <div className="flex-1 bg-white p-4 rounded-xl border border-slate-200 shadow-sm min-h-[140px]">
                            {metodoPago === 'EFECTIVO' && (
                    ...
                    <div className="w-full md:w-1/2 bg-white p-6 flex flex-col shrink-0 relative">
                        ...
                        <div className="flex gap-2 mb-6 p-1 bg-slate-100 rounded-xl shrink-0">
                            <button 
                        ...
                        <div className="mt-6 pt-6 border-t border-slate-100 shrink-0">
                            ...
                            <button 
                                ...
                                disabled={!pagoValido}
                                className={`w-full py-4 rounded-xl font-bold text-white shadow-lg transition-all flex items-center justify-center gap-2 text-lg shrink-0
